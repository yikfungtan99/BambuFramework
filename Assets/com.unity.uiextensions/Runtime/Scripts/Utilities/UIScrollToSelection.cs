...
namespace UnityEngine.UI.Extensions
{
    ...
    public class UIScrollToSelection : MonoBehaviour
    {
        ...
        private void ScrollToSelection(GameObject selection)
        {
            ...
            // Add extra padding to ensure the element isn't clipped
            float paddingPercentage = 0.1f; // 10% padding as an example
            if (Mathf.Abs(offsetToSelection.x) / viewportRectTransform.rect.width >= JumpOffsetThreshold)
            {
                offsetToSelection.x += Mathf.Sign(offsetToSelection.x) * (viewportRectTransform.rect.width * paddingPercentage);
            }
            if (Mathf.Abs(offsetToSelection.y) / viewportRectTransform.rect.height >= JumpOffsetThreshold)
            {
                offsetToSelection.y += Mathf.Sign(offsetToSelection.y) * (viewportRectTransform.rect.height * paddingPercentage);
            }

            // calculate final scroll speed based on the content size
            float finalScrollSpeed = ScrollSpeed;
            float contentWidth = scrollRectContentTransform.rect.width;
            float contentHeight = scrollRectContentTransform.rect.height;

            if (Math.Abs(offsetToSelection.x) / contentWidth >= JumpOffsetThreshold || Math.Abs(offsetToSelection.y) / contentHeight >= JumpOffsetThreshold)
            {
        ...
        private IEnumerator ScrollToPosition(Vector2 targetPosition, float speed)
        {
            ...
            float horizontalSpeed = viewportRectTransform.rect.width * speed;
            float verticalSpeed = viewportRectTransform.rect.height * speed;
